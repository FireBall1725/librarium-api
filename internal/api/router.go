...
func NewRouter(ctx context.Context, db *pgxpool.Pool, cfg *config.Config, riverClient *river.Client[pgx.Tx], metrics MetricsCollector, deps RouterDeps) http.Handler {
	...
	mux.Handle("GET /api/v1/me/pin", requireAuth(http.HandlerFunc(kioskHandler.GetPIN)))
	mux.Handle("PUT /api/v1/me/pin", requireAuth(http.HandlerFunc(kioskHandler.SetPIN)))
	mux.Handle("DELETE /api/v1/me/pin", requireAuth(http.HandlerFunc(kioskHandler.ClearPIN)))
